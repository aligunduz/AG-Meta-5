...
class ResNet18(Module):
  def __init__(self, channels, bn_args):
    ...
    self.layer1 = Sequential(OrderedDict([
      ('block0', Block(64, channels[0], 1, bn_args_dict[1])),
      ('block1', Block(channels[0], channels[0], 1, bn_args_dict[1])),
    ]))
    self.layer2 = Sequential(OrderedDict([
      ('block0', Block(channels[0], channels[1], 2, bn_args_dict[2])),
      ('block1', Block(channels[1], channels[1], 1, bn_args_dict[2])),
    ]))
    self.layer3 = Sequential(OrderedDict([
      ('block0', Block(channels[1], channels[2], 2, bn_args_dict[3])),
      ('block1', Block(channels[2], channels[2], 1, bn_args_dict[3])),
    ]))
    self.layer4 = Sequential(OrderedDict([
      ('block0', Block(channels[2], channels[3], 2, bn_args_dict[4])),
      ('block1', Block(channels[3], channels[3], 1, bn_args_dict[4])),
    ]))
